...
class VenteParAns extends Component {
  constructor(props) {
    super(props);
    this.state = {
      selectedYear: 2024,
  ...
  fetchYData = () => {
    fetch("http://localhost:8080/statNbVoitureVenduAnnee?annee=" + this.state.selectedYear)
      .then((response) => response.json())
      .then((data) => {
        this.setState({ dataY: data }, () => {
          // Call render after the state is updated
          this.chart.render();
        });
      })
      .catch((error) => {
  ...
  handleYearChange = (event) => {
    ...
    this.setState({ selectedYear: newYear }, () => {
      // Call fetchYData with a callback to ensure it's called after the state is updated
      this.fetchYData();
    });
  }
